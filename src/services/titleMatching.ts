...
export function distinctiveTokensExcludingSeason(s: string): string[] {
...
export function detectSeasonMarker(s: string): string {
  const t = s.toLowerCase();

  // e.g. "s2", "s02", "s3", "s03"
  const shortSeason = t.match(/\bs0*(\d+)\b/i) || t.match(/[\s\-_]s0*(\d+)/i);
  if (shortSeason) {
    return String(parseInt(shortSeason[1], 10));
  }
...
export function titleSimilarity(query: string, candidate: string): number {
  ...
  if (qMarker) {
    ...
    if (cMarker === qMarker) {
      score += 0.25;
    } else if (cMarker && cMarker !== qMarker) {
      score -= 0.5; // Heavy penalty for mismatched season!
