...
export const SearchModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 px-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-3xl bg-crafted-surface border border-crafted-border rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        {/* Search Header */}
        <div className="flex items-center px-4 py-3.5 border-b border-crafted-border gap-3 bg-crafted-panel/80">
          <Search className="w-5 h-5 text-crafted-brand-rust" />
          <input
            ref={inputRef}
            type="text"
            value={searchQuery}
            ...
            className="flex-1 bg-transparent text-crafted-text placeholder:text-crafted-text-dim text-base focus:outline-none"
          />
          {searchQuery && (
            <button
          ...
          <button
            ...
            className="px-2.5 py-1 text-xs font-mono rounded-md bg-crafted-border text-crafted-text-muted hover:text-white"
          >
            ESC
          </button>
        </div>
        ...
        {/* Genre Pill Filter */}
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-crafted-border/60 overflow-x-auto no-scrollbar bg-crafted-bg/40">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {searchResults.map((anime) => (
                <div
                  ...
                  className="flex items-center gap-3 p-2.5 rounded-xl bg-crafted-bg/60 border border-crafted-border hover:border-crafted-brand-rust/60 hover:bg-crafted-surface-hover/80 transition-all cursor-pointer group"
                >
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-semibold text-crafted-text truncate group-hover:text-crafted-brand-rustLight transition-colors">
                      {anime.title.english || anime.title.romaji}
                    </h4>
                    <p className="text-xs text-crafted-text-dim truncate mt-0.5">
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-crafted-brand-rust/20 text-crafted-brand-rust hover:bg-crafted-brand-rust hover:text-white transition-colors"
                  >
